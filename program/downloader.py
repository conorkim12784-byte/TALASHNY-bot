...
# نفس مفهوم الـ fallback بتاع الصوت بس للفيديو
_VIDEO_CLIENT_FALLBACK = [
...
def _download_video_with_fallback(link: str):
    """يجرب أكثر من player_client لتحميل الفيديو — بدون cookies"""
    last_err = "فشل التحميل"
    for extractor_args, ua in _VIDEO_CLIENT_FALLBACK:
        ...
        try:
            ...
            last_err = str(e)[:200]
            print(f"[vsong fallback] failed: {last_err}")
            continue
