...
async def play(c: Client, m: Message):
    ...
    if replied:
        if replied.audio or replied.voice:
            ...
            try:
                if replied.audio:
                    songname = getattr(replied.audio, 'title', None) or getattr(replied.audio, 'file_name', 'Audio')
                    songname = songname[:70]
            ...
            if chat_id in QUEUE:
                ...
                image = await thumb(IMG_5, songname, user_id, ctitle)
                pos = add_to_queue(chat_id, songname, dl, link, "Audio", 0)
                requester = f"[{m.from_user.first_name}](tg://user?id={user_id})"
            ...
            if len(m.command) < 2:
                ...
                await _play_youtube(c, m, chat_id, user_id)
    else:
        if len(m.command) < 2:
            await m.reply("» عليك الرد على ** ملف صوتي ** أو ** أكتب شي للبحث**")
        else:
...
async def _play_youtube(c: Client, m: Message, chat_id: int, user_id: int):
    suhu = await c.send_message(chat_id, "🔎 **جاري البحث...**")
    query = m.text.split(None, 1)[1]
    search = ytsearch(query)
    if search == 0:
        return await suhu.edit("❌ **لم يتم العثور على نتائج**")

    songname, url, duration, thumbnail = search
    gcname = m.chat.title
    ctitle = await CHAT_TITLE(gcname)
    image = await thumb(thumbnail, songname, user_id, ctitle)
    veez, ytlink = await ytdl(url)
    if veez == 0:
        return await suhu.edit(f"❌ تم اكتشاف مشاكل في yt-dlp\n\n» `{ytlink}`")
