...
async def cbpause(_, query: CallbackQuery):
    ...
    if not a.privileges or not a.privileges.can_manage_video_chats:
        return await query.answer("💡 يجب أن تكون مشرفاً لاستخدام هذا الزر!", show_alert=True)
    chat_id = query.message.chat.id
    if chat_id in QUEUE:
        try:
            await call_py.pause(chat_id)
            await query.answer("streaming paused")
            await query.edit_message_text("⏸ توقف البث موقتآ", reply_markup=back_mark)
        except Exception as e:
...
async def cbresume(_, query: CallbackQuery):
    ...
    if chat_id in QUEUE:
        try:
            ...
            await query.edit_message_text("▶️ تم استئناف البث", reply_markup=back_mark)
        except Exception as e:
            await query.edit_message_text(f"🚫 **خطأ:**\n\n`{e}`", reply_markup=close_mark)
    else:
        await query.answer("❌ قائمة التشغيل فارغه", show_alert=True)
...
async def cbmute(_, query: CallbackQuery):
    ...
    if chat_id in QUEUE:
        try:
            ...
            await query.edit_message_text("🔇 تم كتم الصوت", reply_markup=back_mark)
        except Exception as e:
            await query.edit_message_text(f"🚫 **خطأ:**\n\n`{e}`", reply_markup=close_mark)
    else:
        await query.answer("❌ قائمة التشغيل فارغه", show_alert=True)
...
async def cbunmute(_, query: CallbackQuery):
    ...
    if chat_id in QUEUE:
        try:
            ...
            await query.edit_message_text("🔊 تم تشغيل الصوت", reply_markup=back_mark)
        except Exception as e:
            await query.edit_message_text(f"🚫 **خطأ:**\n\n`{e}`", reply_markup=close_mark)
    else:
        await query.answer("❌ قائمة التشغيل فارغه", show_alert=True)
